    ...
    if (type == SOUP_WEBSOCKET_DATA_TEXT) { // we are expecting json
        ...
        if (g_strcmp0(msg_type, "data") == 0) {
            ...
            if (g_strcmp0(resource, "/domains/") == 0) {
                ...
                // Удалить при повышении мажорной версии тонкого клиента. Текущая версия тк 1.10.1
                const gchar *event = json_object_get_string_member_safely(root_object, "event");
...
void vdi_ws_client_send_vm_changed(VdiWsClient *ws_vdi_client, const gchar *vm_id, VdiEventType event)
{
    ...
    if (vm_id == NULL || protocol == ANOTHER_REMOTE_PROTOCOL)
        ...
        json_builder_add_string_value(builder, util_remote_protocol_to_str(protocol));
    ...
    if (vm_id == NULL) {
        ...
        gboolean is_connection_secure = (protocol == RDP_PROTOCOL || protocol == RDP_NATIVE_PROTOCOL);
